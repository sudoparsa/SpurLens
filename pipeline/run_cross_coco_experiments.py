...
if __name__ == '__main__':
	...
	from image_mask_datasets import get_image_mask_dataset, CocoCategoryImageMaskDataset
	import argparse
	...
	chunk = args.chunk
	print(f"{coco_idx=}, {mllm_name=}, {img_type=}, {respect_cache=}, {K=}")
	...
	ds = get_image_mask_dataset(f"coco-{coco_idx}")
	class_name = ds.get_class_name()
	...
	with open(os.path.join(COCO_PATH, 'annotations', f'instances_val2017.json'), 'r') as f:
	...
	datasets = {}
	for other_idx in supercats[id_to_supercat[coco_idx]]:
		if coco_idx == other_idx:
			continue
		# datasets[other_idx] = get_image_mask_dataset(f"coco-{other_idx}")
		datasets[other_idx] = CocoCategoryImageMaskDataset(other_idx, 'train', coco=ds.coco, instances_data=ds.instances_data)
	del ds
	
	with open(os.path.join(PIPELINE_STORAGE_DIR, 'cross_rankings', f"coco-{coco_idx}.pkl"), 'rb') as f:
		rankings = pkl.load(f)
	extrema = set()
	for spur_feat in spur_feat_lst:
		if spur_feat == 'floss': continue
		extrema.update([(t[0], t[1]) for t in rankings[spur_feat][:K]])
		extrema.update([(t[0], t[1]) for t in rankings[spur_feat][-K:]])
	...
	pathlib.Path(os.path.join(PIPELINE_STORAGE_DIR, 'cross_experiment_results', f"coco-{coco_idx}", mllm_name)).mkdir(parents=True, exist_ok=True)
	log_filepath = os.path.join(PIPELINE_STORAGE_DIR, 'cross_experiment_results', f"coco-{coco_idx}", mllm_name, f"{img_type}_cross_extra.txt")
	if respect_cache:
		pat = re.compile(r"ds_idx=(\d+), img_idx=(\d+), img_type=(\w+), prompt_id=(\w+)-(\w+)-(\d+) :: res='(.*)'")
		cache = set()
		for other_filename in os.listdir(os.path.join(PIPELINE_STORAGE_DIR, 'cross_experiment_results', f"coco-{coco_idx}", mllm_name)):
			print(f"adding {other_filename=} to cache", flush=True)
			with open(os.path.join(PIPELINE_STORAGE_DIR, 'cross_experiment_results', f"coco-{coco_idx}", mllm_name, other_filename), 'r') as f:
				for line in f.readlines():
					m = pat.match(line.strip())
					if m is not None:
						# print(f"adding to cache: {(int(m[1]), int(m[2]), m[3], m[4], m[5], m[6])}")
	...
	def in_cache(ds_idx: int, img_idx: int, img_type: str, prompt: str) -> bool:
		m = prompt.split('-')
		# print(f"checking cache for {(ds_idx, img_idx, img_type, m[0], m[1], m[2])}", flush=True)
		return ((ds_idx, img_idx, img_type, m[0], m[1], m[2]) in cache)
	

	extrema = sorted(list(extrema))
	num_samples = len(extrema)
	...
	counter = 0
	for extrema_idx in range(chunk_start, chunk_end):
		ds_idx, img_idx = extrema[extrema_idx]
		set_seeds()
		...
		if img_type == 'natural':
			...
			for prompt in prompts:
				...
				try:
					...
					print(f"ERROR PROCESSING {ds_idx=}, {img_idx=}, {prompt['id']=}")
					continue
